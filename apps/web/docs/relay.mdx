...
Este módulo facilita a integração do Relay em sua aplicação Next.js, fornecendo uma configuração robusta para GraphQL.

# Como Utilizar com Duas APIs GraphQL

Agora que você tem dois endpoints GraphQL, você pode usar ambientes Relay separados para cada API.

## Exemplo de Uso

### Para a API principal (localhost:4000):
```tsx
import { ReactRelayContainer } from './relay/ReactRelayContainer';
...
### Para a API acquirer (localhost:4001):
```tsx
import { ReactRelayContainer } from './relay/ReactRelayContainer';
...
export default function AcquirerPage() {
  return (
    <ReactRelayContainer 
      Component={MyAcquirerComponent} 
      props={props} 
      endpoint={process.env.NEXT_PUBLIC_GRAPHQL_ENDPOINT_ACQUIRER}
    />
  );
}
```

### Criando ambientes diretamente:
```typescript
import { createEnvironment } from './relay/environment';

const mainEnv = createEnvironment(); // usa NEXT_PUBLIC_GRAPHQL_ENDPOINT
const acquirerEnv = createEnvironment(process.env.NEXT_PUBLIC_GRAPHQL_ENDPOINT_ACQUIRER);
```

## Notas
- Para subscriptions, continua usando `NEXT_PUBLIC_SUBSCRIPTIONS_ENDPOINT` (atualmente ws://localhost:4001/graphql/ws).
- Certifique-se de que os schemas GraphQL das duas APIs sejam compatíveis ou ajuste conforme necessário.
- Você pode criar contextos separados se precisar de múltiplos ambientes simultaneamente na mesma página.